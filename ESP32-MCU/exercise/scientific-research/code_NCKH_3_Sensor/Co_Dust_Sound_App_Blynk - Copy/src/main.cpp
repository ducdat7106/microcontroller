...
void runBlynks()
{
  ...
  Blynk.virtualWrite(V41, GAS_Value_Use);
}
...
void readDust()
{
  ...
  DUST_Value_Use = DUST_Value_Basic;
...
void readGas()
{
  ...
  GAS_Value_Basic = analogRead(GAS_Pin);
  // SOUND_Value_Use = map(SOUND_Value_Basic, minAnalog, maxAnalog, lowerLimit, upperLimit);
SOUND_Value_Use = GAS_Value_Basic;
  Serial.println("GAS value basic: " + String(SOUND_Value_Basic));
