...
# 2 "<built-in>" 2
# 1 "eeprom_internal.c" 2
# 1 "./eeprom_internal.h" 1
# 14 "./eeprom_internal.h"
...
# 82 "eeprom_internal.c"
void EEPROM_Write_N_Bytes(unsigned char EepromAddr, unsigned char *RamAddr, char NoOfBytes)
{
  while(NoOfBytes != 0)
   {
        EEPROM_Write_Char(EepromAddr,*RamAddr);
        EepromAddr++;
        RamAddr++;
        NoOfBytes--;
 }
...
void EEPROM_Read_String(unsigned char eeprom_address, unsigned char *destination_address)
{
   char eeprom_data;
    ...
    do
    {
        eeprom_data = EEPROM_Read_Char(eeprom_address);
        *destination_address = eeprom_data;
        destination_address++;
        eeprom_address++;
    }while(eeprom_data!=0);
}
# 140 "eeprom_internal.c"
void EEPROM_Read_N_Bytes(unsigned char EepromAddr, unsigned char *RamAddr, char NoOfBytes)
{
 while(NoOfBytes != 0)
 {
        *RamAddr = EEPROM_Read_Char(EepromAddr);
        EepromAddr++;
        RamAddr++;
        NoOfBytes--;
 }
