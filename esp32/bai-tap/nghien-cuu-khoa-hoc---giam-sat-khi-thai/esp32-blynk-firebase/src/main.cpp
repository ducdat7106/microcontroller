...
const char ssid[] = "Viettel Teleco";
...
typedef struct
{
  Sensor Co, Gas, Sound, Dust;
  Led_Hardware connectWifi, messenger_safe, messenger_danger;
} Hardwares;
...
void initWifi(Hardwares hardware);
void initHardware(Hardwares *hardware);
void blink_connect_wifi(Hardwares *hardware, char *blink_mode);
void blink_fast(int pin);
void blink_low(int pin);
...
void loop()
{
  ...
  {
    blink_connect_wifi(&hardware, "slow");
  }
  // runBlynks();
  ...
  // if(WiFi.status() == WL_CONNECTED);
  // blink_low(connectWifi.pin);

  // digitalWrite(hardware.connectWifi.pin, HIGH);
  // delay(1000);
  // digitalWrite(hardware.connectWifi.pin, LOW);
  // delay(1000);
}
...
void blink_fast(int pin)
{
  static unsigned int time_wait, flag = 1;
...
void blink_low(int pin)
{
  static unsigned int time_wait, flag = 0;
  if (millis() - time_wait >= 5000)
  {
    flag = ~flag;
    time_wait = millis();
  }
...
void blink_connect_wifi(Hardwares *hardware, char *blink_mode)
{
  /*
    Have two mode:
      != 0 - slow : Blink slow mode
      == 0 - fast : Blink fast mode
  */
  int value = strcmp("fast", blink_mode);
  static unsigned long time_wait = 0;
  if (value == 0)
    blink_fast(hardware->connectWifi.pin);
  else
    blink_low(hardware->connectWifi.pin);
}
...
void initWifi(Hardwares hardware)
{
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    blink_connect_wifi(&hardware, "fast");
  }
...
{
  // led connect wifi
  hardware->connectWifi.pin = 2;
  hardware->connectWifi.state_current = turn_off;

  pinMode(hardware->connectWifi.pin, OUTPUT);
  digitalWrite(hardware->connectWifi.pin, turn_on);
}
