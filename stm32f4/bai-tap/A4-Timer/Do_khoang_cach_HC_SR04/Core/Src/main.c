...
//Creat variable
HCSR04_State hc_sr04_state = HCSR04_IDLE_STATE;
	...
	switch(hc_sr04_state)
	{
		case HCSR04_WAIT_RISING_STATE:
			if(HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_7) == 1)
			{
				...
				hc_sr04_state = HCSR04_WAIT_FALING_STATE;
				HAL_TIM_Base_Start(&htim1);
			}		
			else
			{
				hc_sr04_state = HCSR04_IDLE_STATE;
			}
				
			break;
		
		case HCSR04_WAIT_FALING_STATE:
			if(HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_7) == 0)
			{
				HAL_TIM_Base_Stop(&htim1);
				hc_sr04_state = HCSR04_COMPLETE_STATE;
			}
			else
			{
				hc_sr04_state = HCSR04_IDLE_STATE;
...
void HC04_Handle()
{
	if(hc_sr04_state == HCSR04_COMPLETE_STATE)
	{
		//tinh toan khoang cach
		float khoangCach = 0.017*htim1.Instance->CNT;
		HC04_Complete_Callback(khoangCach);
		hc_sr04_state = HCSR04_IDLE_STATE;
	}
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		if(HAL_GetTick()-t_get_distance>=300)//300ms
		{
			HC04_Start();
